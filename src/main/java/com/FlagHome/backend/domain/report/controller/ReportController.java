...
public class ReportController {
    ...
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409", description = "이미 신고한 대상입니다")
    })
    @PostMapping
    public ResponseEntity<ApiResponse> report(@RequestBody ReportRequest reportRequest) {
        ...
        ApiResponse response = ApiResponse.of(location, OK, "신고가 접수되었습니다");
    ...
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "모든 신고 가져오기 성공"),
    })
    @GetMapping
    public ResponseEntity<ApiResponse> getAllReports() {
        ApiResponse response = ApiResponse.of(reportService.getAllReports(), OK, "모든 신고 정보 가져오기 성공");
        return ResponseEntity.ok(response);
    }
    ...
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "신고가 삭제되었습니다."),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "신고이력이 존재하지 않습니다.")
    })
    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse> deleteReport(@PathVariable("id") long reportId){
        reportService.deleteReport(reportId);
        ApiResponse response = ApiResponse.of(null, OK, "신고가 삭제되었습니다.");
        return ResponseEntity.ok(response);
    }
    ...
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "모든 신고가 삭제되었습니다."),
    })
    @DeleteMapping
    public ResponseEntity<ApiResponse> deleteAllReports() {
        reportService.deleteAllReports();
        ApiResponse response = ApiResponse.of(null, OK, "모든 신고가 삭제되었습니다.");
        return ResponseEntity.ok(response);
    }
}
